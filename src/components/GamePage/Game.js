...
export default function Game({ name, difficultyLevel, handlePageNavigation }) {
  ...
  const [totalGames, setTotalGames] = useState(
    getFromSessionStorage("totalGames") || []
  );

  const { initialDifficultyFactor, initialRandomWord, initialTimerValue } =
    useCallback(getInitialValues(level));
  ...
  useEffect(() => {
    if (!gameOver) {
      let gameTimeTimer =
        gameTime >= 0 && setInterval(() => setGameTime(gameTime + 1), 1000);
...
const handleTextChange = (e) => {
    ...
    for (let i = 0; i < randomWord.length; i++) {
      if (i < value.length) {
        if (randomWord[i] === value[i]) {
          // Correct character
          highlightedHTML += `<span class="green">${randomWord[i]}</span>`;
        } else {
          // Incorrect character
          highlightedHTML += `<span class="red">${randomWord[i]}</span>`;
        }
      } else {
        // Remaining letters
        highlightedHTML += `<span class="blue">${randomWord[i]}</span>`;
      }
    }
  ...
  if (inputWord === randomWord) {
    ...
    if (wordCount === 50) {
      level === difficultyLevels.EASY
        ? setLevel(difficultyLevels.MEDIUM)
        : setLevel(difficultyLevels.HARD);
      setWordCount(1);
      changeWord();
    } else {
